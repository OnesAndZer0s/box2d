...
#include "box2d/collision/shapes/chain_shape.h"
#include "box2d/collision/shapes/edge_shape.h"
#include "box2d/collision/shapes/shape.h"
#include "box2d/common/block_allocator.h"
#include "box2d/dynamics/body.h"
#include "box2d/dynamics/fixture.h"
#include "box2d/dynamics/world.h"
#include "box2d/dynamics/world_callbacks.h"
...
b2Color* b2ParticleSystem::GetColorBuffer() {
...
void b2ParticleSystem::DestroyOldestParticle(
  ...
  DestroyParticle(
      m_expirationTimeBuffer.data [ oldestFiniteLifetimeParticle ] > 0.0f ? oldestFiniteLifetimeParticle : oldestInfiniteLifetimeParticle,
...
static bool ParticleCanBeConnected(
  ...
  return ( flags & ( b2_wallParticle | b2_springParticle | b2_elasticParticle ) ) ||
...
inline b2ContactFilter* b2ParticleSystem::GetParticleContactFilter() const {
  return ( m_allParticleFlags & b2_particleContactFilterParticle ) ? m_world->m_contactManager.m_contactFilter : NULL;
}
...
class b2ParticleContactRemovePredicate {
  public:
    ...
    bool operator()( const b2ParticleContact& contact ) {
      return ( contact.GetFlags() & b2_particleContactFilterParticle ) && !m_contactFilter->ShouldCollide( m_system, contact.GetIndexA(),
                                                                              contact.GetIndexB() );
    }
...
inline b2ContactListener* b2ParticleSystem::GetFixtureContactListener() const {
  return ( m_allParticleFlags & b2_fixtureContactListenerParticle ) ? m_world->m_contactManager.m_contactListener : NULL;
}
...
inline b2ContactFilter* b2ParticleSystem::GetFixtureContactFilter() const {
  return ( m_allParticleFlags & b2_fixtureContactFilterParticle ) ? m_world->m_contactManager.m_contactFilter : NULL;
}
    ...
    if( IsRigidGroup( aGroup ) ) {
      ...
      // the particle into the body.
      {
        ...
        InitDampingParameter(
            ...
            b->GetWorldCenter(),
            p, n );
        float f = damping * b2Min( w, 1.0f ) * ComputeDampingImpulse( invMassA, invInertiaA, tangentDistanceA, invMassB, invInertiaB, tangentDistanceB, vn );
        ApplyDamping(
            invMassA, invInertiaA, tangentDistanceA,
            true, aGroup, a, f, n );
  ...
  if( colorMixing128 ) {
    for( int32 k = 0; k < m_contactBuffer.GetCount(); k++ ) {
      ...
      if( m_flagsBuffer.data [ a ] & m_flagsBuffer.data [ b ] &
        ...
        b2Color& colorA = m_colorBuffer.data [ a ];
        b2Color& colorB = m_colorBuffer.data [ b ];
        // Use the static method to ensure certain compilers inline
        // this correctly.
        b2Color::MixColors( &colorA, &colorB, colorMixing128 );
      }
    }
  }
}
...
float b2ParticleSystem::ExpirationTimeToLifetime(
  ...
  return (float) ( expirationTime > 0 ? expirationTime - GetQuantizedTimeElapsed() : expirationTime ) * m_def.lifetimeGranularity;
}
...
long long b2ParticleSystem::LifetimeToExpirationTime( const float lifetime ) const {
  return m_timeElapsed + (long long) ( ( lifetime / m_def.lifetimeGranularity ) *
                                       (float) ( 1LL << 32 ) );
}
...
void b2ParticleSystem::SetColorBuffer( b2Color* buffer,
    int32 capacity ) {
