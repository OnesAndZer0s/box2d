...
#include "box2d/common/time_step.h"
#include "particle.h"
...
class b2ParticleSystem {
  public:
    ...
    b2Color* GetColorBuffer();
    const b2Color* GetColorBuffer() const;
    ...
    void SetColorBuffer( b2Color* buffer, int32 capacity );
  ...
  private:
    ...
    template< typename T >
    T* ReallocateBuffer( T* buffer, int32 oldCapacity,
        int32 newCapacity );
    template< typename T >
    T* ReallocateBuffer(
    ...
    long long LifetimeToExpirationTime( const float lifetime ) const;
    ...
    float* m_depthBuffer;
    UserOverridableBuffer< b2Color > m_colorBuffer;
...
inline const b2Color* b2ParticleSystem::GetColorBuffer() const {
...
inline int b2ParticleSystem::CopyColorBuffer( int startIndex,
    ...
    void* outBuf,
    int size ) const {
  int copySize = numParticles * sizeof( b2Color );
